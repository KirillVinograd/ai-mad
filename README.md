...
Одностраничное приложение на Node.js для проведения базовых версий МЛО-АМ («Многоуровневый личностный опросник Адаптивность»), опросника Шмишека, EPI и ПДО. Участник вводит персональные данные и сразу переходит к вопросам; результаты доступны только по закрытой ссылке `/tests/:slug/results`.
