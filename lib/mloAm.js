...
function computeRawScore(items, answers) {
  return (items || []).reduce((acc, item) => {
    ...
    const expected = typeof item === 'object' && item !== null && item.answer_value ? item.answer_value : 'yes';
    if (!questionId) return acc;
    return acc + (answers[questionId] === expected ? 1 : 0);
  }, 0);
}
...
function findNorm(finalScore, norms) {
  ...
  const belowRange = norms.table[0] && finalScore < norms.table[0].min ? norms.table[0] : null;
  const aboveRange =
    norms.table[norms.table.length - 1] && finalScore > norms.table[norms.table.length - 1].max
      ? norms.table[norms.table.length - 1]
      : null;
  const target = found || belowRange || aboveRange || null;
  if (target) {
...
function orderScaleIds(scales) {
  const order = ['D', 'A', 'B', 'C', 'I', 'G', 'H', 'L2', 'L', 'F', 'K'];
...
function buildDerived(scales, scoring) {
  ...
  const lapGroup = lap
    ? lap.level && lap.level.startsWith('группа')
      ? lap.level
